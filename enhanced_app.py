...
def predict_stock_price(ticker):
    ...
    try:
        ...
        if len(data) < 60:
            logger.error(f"Insufficient data for {ticker}, padding with synthetic data")
            # Pad with synthetic data
            padding = np.random.normal(data.mean(), data.std(), 60 - len(data))
        ...
        prices = scaler.inverse_transform(data[0][-30:].reshape(-1, 1)).flatten()
        ...
        # Day 2 prediction (with more uncertainty)
        day2_price = day1_price * (1 + (day1_change / 200))
        day2_change = day1_change * 1.2
        day2_confidence = day1_confidence * 0.9
        
        # Day 3 prediction (with even more uncertainty)
        day3_price = day2_price * (1 + (day2_change / 200))
        day3_change = day2_change * 1.2
        ...
        predictions = [
            {
                ...
                "change_percent": float(day1_change),
            ...
            },
            {
                ...
                "change_percent": float(day2_change),
            ...
            },
            {
                ...
                "change_percent": float(day3_change),
