    ...
    <aside className={`bg-indigo-700 text-white ${expanded ? 'w-64' : 'w-20'} transition-all duration-300 flex flex-col`}>
      <div className="flex items-center p-4 border-b border-indigo-600">
        {expanded ? (
          ...
          <button onClick={toggleSidebar} className="w-full text-white focus:outline-none">
        ...
        <ul className="space-y-2">
          {navItems.map((item) => (
            <li key={item.name}>
              ...
              >
                <div className="flex items-center">
                  <div className={expanded ? 'mr-3' : 'mx-auto'}>{item.icon}</div>
